<template>
  <footer class="bg-white dark:bg-gray-900 border-t border-gray-200 dark:border-gray-700 mt-20">
    <div class="container mx-auto px-4 py-12">
      <div class="grid grid-cols-1 md:grid-cols-3 gap-8">
        <!-- Brand -->
        <div class="col-span-1">
          <h3 class="text-2xl font-bold gradient-text mb-4">我的博客</h3>
          <p class="text-gray-600 dark:text-gray-400 mb-4">
            记录技术成长，分享编程心得。专注于前端开发、Vue.js 和现代 Web 技术。
          </p>
          <div class="flex space-x-4">
            <a href="https://github.com/JinshuaiJiao" target="_blank"
              class="text-gray-500 hover:text-primary-600 dark:text-gray-400 dark:hover:text-primary-400 transition-colors"
              title="GitHub">
              <svg class="w-6 h-6" fill="currentColor" viewBox="0 0 24 24">
                <path
                  d="M12 0c-6.626 0-12 5.373-12 12 0 5.302 3.438 9.8 8.207 11.387.599.111.793-.261.793-.577v-2.234c-3.338.726-4.033-1.416-4.033-1.416-.546-1.387-1.333-1.756-1.333-1.756-1.089-.745.083-.729.083-.729 1.205.084 1.839 1.237 1.839 1.237 1.07 1.834 2.807 1.304 3.492.997.107-.775.418-1.305.762-1.604-2.665-.305-5.467-1.334-5.467-5.931 0-1.311.469-2.381 1.236-3.221-.124-.303-.535-1.524.117-3.176 0 0 1.008-.322 3.301 1.23.957-.266 1.983-.399 3.003-.404 1.02.005 2.047.138 3.006.404 2.291-1.552 3.297-1.23 3.297-1.23.653 1.653.242 2.874.118 3.176.77.84 1.235 1.911 1.235 3.221 0 4.609-2.807 5.624-5.479 5.921.43.372.823 1.102.823 2.222v3.293c0 .319.192.694.801.576 4.765-1.589 8.199-6.086 8.199-11.386 0-6.627-5.373-12-12-12z" />
              </svg>
            </a>
            <a href="https://blog.csdn.net/m0_71349739?spm=1000.2115.3001.5343"
              class="text-gray-500 hover:text-primary-600 dark:text-gray-400 dark:hover:text-primary-400 transition-colors"
              title="Email">
              <svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                  d="M3 8l7.89 4.26a2 2 0 002.22 0L21 8M5 19h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v10a2 2 0 002 2z" />
              </svg>
              <!-- <img class="w-6 h-6" src="https://t15.baidu.com/it/u=2005731947,4139443793&fm=195&app=88&size=r1,1&q=100&n=0&g=4n&f=JPEG&maxorilen2heic=2000000" alt=""> -->
            </a>
          </div>
        </div>

        <!-- Quick Links -->
        <div class="col-span-1">
          <h4 class="text-lg font-semibold mb-4 text-gray-900 dark:text-gray-100">快速链接</h4>
          <ul class="space-y-2">
            <li>
              <router-link to="/" class="footer-link">首页</router-link>
            </li>
            <li>
              <router-link to="/about" class="footer-link">关于我</router-link>
            </li>
            <li>
              <a href="#" class="footer-link">RSS订阅</a>
            </li>
          </ul>
        </div>

        <!-- Categories -->
        <div class="col-span-1">
          <h4 class="text-lg font-semibold mb-4 text-gray-900 dark:text-gray-100">分类</h4>
          <ul class="space-y-2">
            <li v-for="category in blogStore.categories.slice(0, 5)" :key="category.id">
              <router-link :to="`/category/${category.id}`" class="footer-link">
                {{ category.name }}
              </router-link>
            </li>
          </ul>
        </div>
      </div>

      <div class="border-t border-gray-200 dark:border-gray-700 mt-8 pt-8 text-center">
        <p class="text-gray-600 dark:text-gray-400">
          © {{ new Date().getFullYear() }} 我的博客. 所有权利保留.
        </p>
      </div>
    </div>
  </footer>
</template>

<script setup lang="ts">
import { useBlogStore } from '../stores/blog'

const blogStore = useBlogStore()
</script>

<style scoped>
.footer-link {
  @apply text-gray-600 dark:text-gray-400 hover:text-primary-600 dark:hover:text-primary-400 transition-colors;
}
</style>